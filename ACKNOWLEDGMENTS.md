# Acknowledgments

Este proyecto existe gracias al trabajo de muchas personas y comunidades open source.

## Gentleman Programming

Agradecimiento especial a [Alan Buscaglia](https://github.com/Gentleman-Programming) y la comunidad de Gentleman Programming por:

- **[Engram](https://github.com/Gentleman-Programming/engram)** — Sistema de memoria persistente para agentes de IA
- **[Gentle AI](https://github.com/Gentleman-Programming/gentle-ai)** — Ecosistema de configuración y orquestación SDD
- **[Gentleman Skills](https://github.com/Gentleman-Programming/gentleman-skills)** — Formato de skills y colección curada
- **[OpenSpec](https://openspec.dev)** — Especificación para desarrollo guiado por especificaciones

El formato de skills de este proyecto es compatible con el ecosistema Gentleman, y la integración SDD fue diseñada para complementar su orquestador.

## Model Context Protocol (MCP)

- **[MCP SDK](https://github.com/modelcontextprotocol/typescript-sdk)** — Anthropic — MIT License
  El protocolo y SDK que hace posible que este servidor se conecte con cualquier agente de IA.

## Dependencias directas

| Paquete | Autor/Org | Licencia |
|---|---|---|
| [@modelcontextprotocol/sdk](https://github.com/modelcontextprotocol/typescript-sdk) | Anthropic | MIT |
| [zod](https://github.com/colinhacks/zod) | Colin McDonnell | MIT |
| [typescript](https://github.com/microsoft/TypeScript) | Microsoft | Apache-2.0 |

## Tecnologías para las que este proyecto provee inteligencia

Este MCP server está diseñado para proyectos que usan estas tecnologías. Agradecemos a sus comunidades:

| Tecnología | Org/Autor | Licencia |
|---|---|---|
| [Next.js](https://github.com/vercel/next.js) | Vercel | MIT |
| [React](https://github.com/facebook/react) | Meta | MIT |
| [Prisma](https://github.com/prisma/prisma) | Prisma | Apache-2.0 |
| [NextAuth.js](https://github.com/nextauthjs/next-auth) | Balázs Orbán et al. | ISC |
| [Zod](https://github.com/colinhacks/zod) | Colin McDonnell | MIT |
| [shadcn/ui](https://github.com/shadcn-ui/ui) | shadcn | MIT |
| [Tailwind CSS](https://github.com/tailwindlabs/tailwindcss) | Tailwind Labs | MIT |
| [Vitest](https://github.com/vitest-dev/vitest) | Anthony Fu et al. | MIT |

## Inspiración

- **[Everything Claude Code](https://github.com/affaan-m/everything-claude-code)** — Affaan M — MIT License
  El skill de seguridad (`skills/security.md`) fue inspirado y adaptado a partir de su `security-review` skill, ajustado al stack Next.js + Prisma de este proyecto.

## Comunidad

Si querés contribuir, abrí un issue o un PR. Toda contribución es bienvenida.
